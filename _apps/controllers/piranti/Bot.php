...
class Bot extends CI_Controller {
	...
	function generateUserDesa(){
		...
		foreach ($query->result_array() as $rs){
			...
			$email = fixNamaUrl($strNama)."@".$kabupaten."kab.go.id";
			// echo "<br />".fixNamaUrl($strNama).".wonogiri";
			$strSQL = "('887','".fixSQL($nama)."','".fixSQL($userid)."','".password_hash($strPassword,PASSWORD_DEFAULT)."',3,'".fixSQL($email)."',1,'".$rs['kode']."'), \n";
			echo $strSQL;
		}
		echo "</pre>";
	...
	function import_bdt_rts(){
		...
		if($query){
			if($query->num_rows() > 0){
				...
				// SQL KRA
				// $strSQL = "SELECT * FROM ".$sumber." WHERE DESA_KODE='".fixSQL($desa['kode'])."'";
				
				// SQL WNG
				$strSQL = "SELECT * FROM ".$sumber." WHERE kode_wilayah='".fixSQL($desa['kode'])."'";
				// echo $strSQL;
				$query = $this->db->query($strSQL);
				if($query){
					...
					// utk seliain 2019 tidak pake ke tweb_rumahtangga 
					...
					if($numrows > 0){
						foreach ($query->result_array() as $rs){
							...
							$strSQLBDT .="($periode_id,'".fixSQL($kode_wilayah)."','".fixSQL($kd_desa)."','".fixSQL($kd_prop)."','".fixSQL($kd_kab)."','".fixSQL($kd_kec)."','".fixSQL($kd_desa)."','".fixSQL($rs['IDBDT'])."', '".fixSQL($rs['Alamat'])."', '".fixSQL($rs['NoPesertaPKH'])."', '".fixSQL($rs['Nama_SLS'])."', '".fixSQL($rs['Nama_KRT'])."', '".fixSQL($rs['Jumlah_ART'])."', '".fixSQL($rs['Jumlah_Keluarga'])."', '".fixSQL($rs['sta_bangunan'])."', '".fixSQL($rs['sta_lahan'])."', '".fixSQL($rs['luas_lantai'])."', '".fixSQL($rs['lantai'])."', '".fixSQL($rs['dinding'])."', '".fixSQL($rs['kondisi_dinding'])."', '".fixSQL($rs['atap'])."', '".fixSQL($rs['kondisi_atap'])."', '".fixSQL($rs['jumlah_kamar'])."', '".fixSQL($rs['sumber_airminum'])."', '".fixSQL($rs['nomor_meter_air'])."', '".fixSQL($rs['cara_peroleh_airminum'])."', '".fixSQL($rs['sumber_penerangan'])."', '".fixSQL($rs['daya'])."', '".fixSQL($rs['nomor_pln'])."', '".fixSQL($rs['bb_masak'])."', '".fixSQL($rs['nomor_gas'])."', '".fixSQL($rs['fasbab'])."', '".fixSQL($rs['kloset'])."', '".fixSQL($rs['buang_tinja'])."', '".fixSQL($rs['ada_tabung_gas'])."', '".fixSQL($rs['ada_lemari_es'])."', '".fixSQL($rs['ada_ac'])."', '".fixSQL($rs['ada_pemanas'])."', '".fixSQL($rs['ada_telepon'])."', '".fixSQL($rs['ada_tv'])."', '".fixSQL($rs['ada_emas'])."', '".fixSQL($rs['ada_laptop'])."', '".fixSQL($rs['ada_sepeda'])."', '".fixSQL($rs['ada_motor'])."', '".fixSQL($rs['ada_mobil'])."', '".fixSQL($rs['ada_perahu'])."', '".fixSQL($rs['ada_motor_tempel'])."', '".fixSQL($rs['ada_perahu_motor'])."', '".fixSQL($rs['ada_kapal'])."', '".fixSQL($rs['aset_tak_bergerak'])."', '".fixSQL($rs['luas_atb'])."', '".fixSQL($rs['rumah_lain'])."', '".fixSQL($rs['jumlah_sapi'])."', '".fixSQL($rs['jumlah_kerbau'])."', '".fixSQL($rs['jumlah_kuda'])."', '".fixSQL($rs['jumlah_babi'])."', '".fixSQL($rs['jumlah_kambing'])."', '".fixSQL($rs['sta_art_usaha'])."', '".fixSQL($rs['sta_kks'])."', '".fixSQL($rs['sta_kip'])."', '".fixSQL($rs['sta_kis'])."', '".fixSQL($rs['sta_bpjs_mandiri'])."', '".fixSQL($rs['sta_jamsostek'])."', '".fixSQL($rs['sta_asuransi'])."', '".fixSQL($rs['sta_pkh'])."', '".fixSQL($rs['sta_rastra'])."', '".fixSQL($rs['sta_kur'])."', '".fixSQL($rs['sta_keberadaan_RT'])."', '".fixSQL($rs['percentile'])."')".$strKoma;
					...
					// echo "<h1>RT</h1>";
					// echo var_dump($rts);
					$file_sql_bdt = FCPATH ."assets/uploads/".$folder."/bdt_".$sumber."-".$kode_desa.".sql";
					// $file_sql_rts = FCPATH ."assets/uploads/".$folder."/rts_".$sumber."-".$kode_desa.".sql";
					...
					$strSQL = "UPDATE tweb_wilayah SET proses=1 WHERE kode='".$kode_desa."'";
					if($this->db->query($strSQL)){
						echo $strSQL;
					}
	...
	function import_dtks_2018(){
		// KARANGANYAR
		$periode_id = 2;
		$kode_base = "3313";
		// $sumber = "kra_dtks_idv_2019";
		$sumber = "kra_idv_2018";
		$folder = "kra2018/idv/";
		...
		// $kode_base = "3312";
		// $sumber = "wng_art_20191";
		// $folder = "wng2019/penduduk";

		$strSQL = "SELECT id,kode,nama FROM tweb_wilayah WHERE tingkat=4 AND kode LIKE '".$kode_base."%' AND proses=0 LIMIT 1";
		$query = $this->db->query($strSQL);
		if($query){
			if($query->num_rows() > 0){
				$desa = $query->result_array()[0];
				...
				$kode_wilayah = array();
				$rts_percentile = array();
				$strSQL = "SELECT DISTINCT(idbdt),kode_wilayah,percentile FROM bdt_rts WHERE kode_wilayah LIKE '".$kode_base."%' GROUP BY idbdt";
				$query = $this->db->query($strSQL);
				if($query){
					foreach($query->result() as $rs){
						$output = preg_replace( '/[^0-9]/', '', $rs->kode_wilayah);
						$idbdtnya = preg_replace( '/[^0-9]/', '', $rs->idbdt);
						$kode_wilayah[$idbdtnya] = $output;
						$rts_percentile[$idbdtnya] = $rs->percentile;
					}
				}
				// echo var_dump($kode_wilayah);
				// echo count($rts_percentile);

				$file_sql_rts = FCPATH ."assets/uploads/".$folder."/pdd_".$sumber."_".$kode_desa.".sql";
				if(is_file($file_sql_rts)){
				}else{
					$newfile = fopen($file_sql_rts,"w");
					fclose($newfile);
				...
				// $strSQL = "SELECT `IDARTBDT`, `IDBDT`, `NoPesertaPKH`, `NoPesertaPBI`, `NoArtPBI`, `Nama`, `JnsKel`, `TmpLahir`, `TglLahir`, `NIK`, `NoKK`, `Hub_KRT`, `NUK`, `Hubkel`, `Umur`, `Sta_kawin`, `Ada_akta_nikah`, `Ada_diKK`, `Ada_kartu_identitas`, `Ada_kks`, `Sta_hamil`, `Jenis_cacat`, `Penyakit_kronis`, `Partisipasi_sekolah`, `Pendidikan_tertinggi`, `Kelas_tertinggi`, `Ijazah_tertinggi`, `Sta_Bekerja`, `Jumlah_jamkerja`, `Lapangan_usaha`, `Status_pekerjaan` FROM ".$sumber." WHERE DESA_KODE='".fixSQL($desa['kode'])."' ORDER BY id";
				// echo $strSQL;

				// `IDARTBDT`, `IDBDT`, `RUTA6`, `NoPesertaPBDT`, `KDPROP`, `KDKAB`, `KDKEC`, `KDDESA`, `NoPesertaPKH`, `NoPesertaKKS2016`, `NoPesertaPBI`, `NoArtPBI`, `Nama`, `JnsKel`, `TmpLahir`, `TglLahir`, `NIK`, `NoKK`, `Hub_KRT`, `NUK`, `Hubkel`, `Umur`, `Sta_kawin`, `Ada_akta_nikah`, `Ada_diKK`, `Ada_kartu_identitas`, `Sta_hamil`, `Jenis_cacat`, `Penyakit_kronis`, `Partisipasi_sekolah`, `Pendidikan_tertinggi`, `Kelas_tertinggi`, `Ijazah_tertinggi`, `Sta_Bekerja`, `Jumlah_jamkerja`, `Lapangan_usaha`, `Status_pekerjaan`, `IDPENGURUS`, `Alamat_Pengurus`, `MasukKuota`
				// $strSQL = "SELECT `IDARTBDT`, `IDBDT`, `NoPesertaPKH`, `NoPesertaPBI`, `NoArtPBI`, `Nama`, `JnsKel`, `TmpLahir`, `TglLahir`, `NIK`, `NoKK`, `Hub_KRT`, `NUK`, `Hubkel`, `Umur`, `Sta_kawin`, `Ada_akta_nikah`, `Ada_diKK`, `Ada_kartu_identitas`, `Ada_kks`, `Sta_hamil`, `Jenis_cacat`, `Penyakit_kronis`, `Partisipasi_sekolah`, `Pendidikan_tertinggi`, `Kelas_tertinggi`, `Ijazah_tertinggi`, `Sta_Bekerja`, `Jumlah_jamkerja`, `Lapangan_usaha`, `Status_pekerjaan` FROM ".$sumber." WHERE kode_wilayah='".fixSQL($desa['kode'])."' ORDER BY id";
				$strSQL = "SELECT `IDARTBDT`, `IDBDT`, `NoPesertaPKH`, `NoPesertaPBI`, `NoArtPBI`, `Nama`, `JnsKel`, `TmpLahir`, `TglLahir`, `NIK`, `NoKK`, `Hub_KRT`, `NUK`, `Hubkel`, `Umur`, `Sta_kawin`, `Ada_akta_nikah`, `Ada_diKK`, `Ada_kartu_identitas`, `Sta_hamil`, `Jenis_cacat`, `Penyakit_kronis`, `Partisipasi_sekolah`, `Pendidikan_tertinggi`, `Kelas_tertinggi`, `Ijazah_tertinggi`, `Sta_Bekerja`, `Jumlah_jamkerja`, `Lapangan_usaha`, `Status_pekerjaan` FROM ".$sumber." WHERE kode_wilayah='".fixSQL($desa['kode'])."' ORDER BY id";
				echo $strSQL;
				...
				if($query){
					...
					if($numrows > 0){
						...
						foreach ($query->result_array() as $rs){
							$key_idbdt = preg_replace( '/[^0-9]/', '', $rs['IDBDT']);
							$percentile = (array_key_exists($key_idbdt,$rts_percentile)) ? $rts_percentile[$key_idbdt]:0;

							if(fmod($nomer,300)==0){
								$strSQLData = "($periode_id, '".fixSQL($kode_wilayah[$key_idbdt])."', '".fixSQL($rs['IDARTBDT'])."', '".fixSQL($key_idbdt)."', '".fixSQL($rs['IDARTBDT'])."', '".fixSQL($kd_prop)."', '".fixSQL($kd_kab)."', '".fixSQL($kd_kec)."', '".fixSQL($kd_desa)."', '".fixSQL($rs['NoPesertaPKH'])."', '".fixSQL($rs['NoPesertaPBI'])."', '".fixSQL($rs['IDARTBDT'])."', '".fixSQL($rs['NoPesertaPBI'])."', '".fixSQL($rs['Nama'])."', '".fixSQL($rs['JnsKel'])."', '".fixSQL($rs['TmpLahir'])."', '".fixSQL($rs['TglLahir'])."', '".fixSQL($rs['Hub_KRT'])."', '".fixSQL($rs['NIK'])."', '".fixSQL($rs['NoKK'])."', '".fixSQL($rs['Hub_KRT'])."', '".fixSQL($rs['NUK'])."', '".fixSQL($rs['Hubkel'])."', '".fixSQL($rs['Umur'])."', '".fixSQL($rs['Sta_kawin'])."', '".fixSQL($rs['Ada_akta_nikah'])."', '".fixSQL($rs['Ada_diKK'])."', '".fixSQL($rs['Ada_kartu_identitas'])."', '".fixSQL($rs['Sta_hamil'])."', '".fixSQL($rs['Jenis_cacat'])."', '".fixSQL($rs['Penyakit_kronis'])."', '".fixSQL($rs['Partisipasi_sekolah'])."', '".fixSQL($rs['Pendidikan_tertinggi'])."', '".fixSQL($rs['Kelas_tertinggi'])."', '".fixSQL($rs['Ijazah_tertinggi'])."', '".fixSQL($rs['Sta_Bekerja'])."', '".fixSQL($rs['Jumlah_jamkerja'])."', '".fixSQL($rs['Lapangan_usaha'])."', '".fixSQL($rs['Status_pekerjaan'])."', '".fixSQL($percentile)."');\n";
								$sqlSave .= $strSQLData;
								$sqlSave .= $strSQLART;
					...
					$strSQL = "UPDATE tweb_wilayah SET proses='1' WHERE id='".fixSQL($desa_id)."'";
					$query = $this->db->query($strSQL);
					if($query){
						echo date("Y-m-d H:i:s")." Ada $nomer data dlm ".$insert." Query \n".$strSQL;

					}else{
						echo date("Y-m-d H:i:s")." LHO KOK GA PROSES?\n".$strSQL;
	...
	function import_bdt_art(){
		...
		$file_counter = FCPATH ."assets/uploads/kra2019/pdd_nomer.txt";
		if(is_file($file_counter)){
		}else{
			$newfile = fopen($file_counter,"w");
			fclose($newfile);
		...
		$kode_wilayah = array();
		$strSQL = "SELECT DISTINCT(idbdt),kode_wilayah FROM bdt_rts WHERE (`kode_wilayah` LIKE '".$kode_base."%') AND (`periode_id`=4) GROUP BY idbdt";
		$query = $this->db->query($strSQL);
		if($query){
			foreach($query->result() as $rs){
				$output = preg_replace( '/[^0-9]/', '', $rs->kode_wilayah);
		...
		$file_sql_rts = FCPATH ."assets/uploads/kra2019/penduduk/".$sumber."_".$counter.".sql";
		if(is_file($file_sql_rts)){
		}else{
			$newfile = fopen($file_sql_rts,"w");
			fclose($newfile);
		...
		$strSQL = "SELECT a.* 
			FROM ".$sumber." a
			WHERE (a.`kode_wilayah` LIKE '".$kode_base."%') 
			AND a.periode_id=4 
			AND a.idartbdt NOT IN (SELECT idartbdt b FROM tweb_penduduk b WHERE (b.`kode_wilayah` LIKE '".$kode_base."%')) 
			LIMIT ".$offset.",".$limit;
		// echo $strSQL;
		$query = $this->db->query($strSQL);
		if($query){
			// $strSQLBDT = "INSERT INTO tweb_keluarga(`kode_wilayah`,) VALUES\n";
			$strSQLART = "INSERT INTO tweb_penduduk(`kode_wilayah`, `idartbdt`, `idbdt`, `ruta6`, `nopesertapbdt`, `nopesertapbdtart`, `nik`, `nama`, `kk_nomor`, `rt_hubungan`, `kk_hubungan`, `tlahir`, `dtlahir`, `jnskel`) VALUES \n";
			// fwrite($fp,$strSQLART);
			$sqlSave = $strSQLART;
			...
			if($numrows > 0){
				foreach ($query->result_array() as $rs){
					$dtlahir = ($rs['tgllahir'] == 'NULL') ? '0000-00-00':$rs['tgllahir'];
					if(fmod($nomer,600)==0){
						$strSQLData = "('".fixSQL($kode_wilayah[$rs['idbdt']])."', '".fixSQL($rs['idartbdt'])."', '".fixSQL($rs['idbdt'])."', '".fixSQL($rs['ruta6'])."', '".fixSQL($rs['nopesertapbdt'])."', '".fixSQL($rs['nopesertapbdtart'])."', '".fixSQL($rs['nik'])."', '".fixSQL($rs['nama'])."', '".fixSQL($rs['nokk'])."', '".fixSQL($rs['hub_krt'])."', '".fixSQL($rs['hubkel'])."', '".fixSQL($rs['tmplahir'])."', '".fixSQL($dtlahir)."', '".fixSQL($rs['jnskel'])."');\n";
						$sqlSave .=$strSQLData;
						$sqlSave .= $strSQLART;
						// fwrite($fp,$strSQLData);
						// fwrite($fp,$strSQLART);
	...
	function ambil_wilayah_capil(){
		$url = "http://118.97.16.131:8282/index.html?akses=kec&nomor_prop=33&nomor_kab=13";
		$data = file_get_contents($url);
		$data_kecamatan = json_decode($data);

		$data_capil = array();
		foreach($data_kecamatan->data as $rs){
			$w = (array) $rs;
			// echo "->". $w['nama_kec']."\n";
			$no_kec = trim($w['no_kec']) * 10;
			$data_capil[trim($w['nama_kec'])] = array(
				'kode'=>trim($w['no_prop']).trim($w['no_kab']).str_pad($no_kec,3,'0',STR_PAD_LEFT),
				'kode_kec' => trim($w['no_kec']),
				'nama'=>$w['nama_kec']
			);
			
		}
		// $strSQL = 
		// echo var_dump($data_capil);
		$strSQL = "SELECT * FROM tweb_wilayah WHERE kode LIKE '3313%' AND tingkat=4";
		$query = $this->db->query($strSQL);
		$desa_siteman = array();
		if($query){
			if($query->num_rows() > 0){
				foreach($query->result_array() as $rs){
					$desa_siteman[substr($rs['kode'],0,7)][$rs['nama']] = array(
							'kode'=>$rs['kode'],
							'nama'=>$rs['nama'],
					);
				}
		...
		$strSQL = "SELECT * FROM tweb_wilayah WHERE kode LIKE '3313%' AND tingkat=3";
		$query = $this->db->query($strSQL);
		if($query){
			if($query->num_rows() > 0){
				$strSQLDesa = "";
				...
				foreach($query->result_array() as $rs){
					...
					$color = ($rs['kode'] == $data_capil[$rs['nama']]['kode']) ? "style='color:#cc0;'":"";

					echo "<h2 ".$color.">WILAYAH KECAMATAN : ".$rs['kode']." : Kec. ".$rs['nama']." -> CAPIL ".$data_capil[$rs['nama']]['kode']." ".$data_capil[$rs['nama']]['nama']."</h2>
					<ul>";
					$strSQLKecamatan .= "UPDATE tweb_wilayah SET kode_capil='".fixSQL($data_capil[$rs['nama']]['kode'])."' WHERE kode='".fixSQL($rs['kode'])."';\n";

					$url = "http://118.97.16.131:8282/index.html?akses=kel&nomor_prop=33&nomor_kab=13&nomor_kec=".$data_capil[$rs['nama']]['kode_kec'];
					echo $url."\n";
					$data = file_get_contents($url);
					$data_kelurahan = json_decode($data);
					// echo var_dump($data_kelurahan);
					foreach($data_kelurahan->data as $desaku){
						$desa = (array) $desaku;
						$no_kec = $desa['no_kec'] * 10;
						$no_kec = str_pad($no_kec,3,0,STR_PAD_LEFT);
						$kode_kec = $desa['no_prop'].$desa['no_kab'].$no_kec;
						$kode_desa = $desa['no_prop'].$desa['no_kab'].$no_kec.$desa['no_kel'];
						$kode_desa_x = $desa['no_prop'].$desa['no_kab'].$no_kec.substr($desa['no_kel'],-3);
						$coloring = ($kode_desa_x == $desa_siteman[$kode_kec][$desa['nama_kel']]['kode'])? "style='color:green'":"";
						echo "<li ".$coloring.">
						Data CAPIL: ".$desa['nama_kel']." => ".$kode_desa."\n
						Data WILAYAH: ".$desa_siteman[$kode_kec][$desa['nama_kel']]['nama']." :> ".$desa_siteman[$kode_kec][$desa['nama_kel']]['kode']."
						</li>";
						$strSQLDesa .= "UPDATE tweb_wilayah SET kode_capil='".fixSQL($kode_desa)."' WHERE kode='".fixSQL($desa_siteman[$kode_kec][$desa['nama_kel']]['kode'])."';\n";
						// echo var_dump($desa);
					}
					echo "</ul>";
				}

				echo "<pre>".$strSQLKecamatan."</pre>";
				echo "<pre>".$strSQLDesa."</pre>";
	...
	function fix_tanggal_penduduk(){
		$strSQL = "SELECT id,nik FROM tweb_penduduk WHERE kode_wilayah_capil LIKE '3313%'";
		$query = $this->db->query($strSQL);
		if($query){
			if($query->num_rows() > 0){
				foreach($query->result_array() as $rs){
					$dt = $rs['nik']; 
					// 331304 67 09 74 0003
					$tahun = (int) substr($dt,10,2);
					$tahun = ($tahun > 20) ? $tahun + 1900: $tahun + 2000;
					$bulan = (string) substr($dt,8,2);
					$tanggal = (int) substr($dt,6,2);
					$tanggal = ($tanggal > 40) ? $tanggal - 40:$tanggal;
					$dtlahir = $tahun ."-".$bulan."-".str_pad($tanggal,2,0,STR_PAD_LEFT);
					$strSQL = "UPDATE tweb_penduduk SET dtlahir='".$dtlahir."',tgl_lahir='".$dtlahir."' WHERE id=".$rs['id']."\n";
					//  AND nik='".$rs['nik']."'; \n";
					echo $strSQL;
				}
			}
	...
	function create_crontab(){
		...
		/usr/bin/php /home/isnusun/pbdt.web.id/bot/kabupaten/index.php piranti/wscapil bot2019_desa 3313150003 >> /home/isnusun/pbdt.web.id/bot/bot_capil.log";

		$strSQL = "SELECT kode,nama FROM tweb_wilayah WHERE tingkat=4 AND kode LIKE '3313%' ORDER BY kode";
		$query = $this->db->query($strSQL);
		if($query){
			if($query > 0){
				foreach ($query->result_array() as $rs) {
					// 
					# code...
					if(substr($rs['kode'],0,7) != '3313150'){
						// echo substr($rs['kode'],0,7);

						$name_berkas = "botcapil_".$rs['kode']."_".fixNamaUrl($rs['nama']).".sh";
						$file_sql_rts = FCPATH ."src/bot/krabaru/".$name_berkas;
						if(is_file($file_sql_rts)){
						}else{
							$newfile = fopen($file_sql_rts,"w");
							fclose($newfile);
...
/usr/bin/php /home/isnusun/pbdt.web.id/bot/kabupaten/index.php piranti/wscapil bot2019_desa ".$rs['kode']." >> /home/isnusun/pbdt.web.id/bot/bot_capil_".$rs['kode']."_".fixNamaUrl($rs['nama']).".log";
	
						// echo $strText;
						fwrite($fp,$strText);
						echo "* * * * * /home/isnusun/pbdt.web.id/bot/".$name_berkas." \n";
						unset($fp);
