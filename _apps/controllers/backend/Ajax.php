...
class Ajax extends CI_Controller {
	...
	function rumahtangga_dan_kepala(){
		...
		$varWilayah = ($user['tingkat'] >= 3)? $user['wilayah']:KODE_BASE;

		if(strlen($strCari) >= 3){
			$data = $this->kependudukan_model->cari_rtm($strCari,$varWilayah);
			header('Content-Type: application/json');
	...
	function kartu_keluarga(){
		...
		if(strlen($strCari) >= 5){
			$data = $this->kependudukan_model->cari_kartukeluarga(trim($strCari));
			header('Content-Type: application/json');
			echo "{ \"total_count\": ".count($data).", \"incomplete_results\": false, \"items\":".json_encode($data)."}";
		}
	}
	...
	function penduduk(){
		...
		if(strlen($strCari) >= 3){
			//$data = $this->kependudukan_model->cari_rtm($strCari,$varWilayah);
			$data = $this->kependudukan_model->cari_art2rtm($strCari,$varWilayah);
			header('Content-Type: application/json');
			echo "{ \"total_count\": ".count($data).", \"incomplete_results\": false, \"items\":".json_encode($data)."}";
		}
	}
	...
	function json_subwilayah($varKode){
		$this->load->model('wilayah_model');
		$n = strlen($varKode);
		$parent = $this->wilayah_model->get_wilayah($varKode);
		$data = $this->wilayah_model->subwilayah($varKode);
		foreach($data as $key=>$item){
			$out[] = array('id'=>$key, 'name'=>$item['nama'] ." - ".$parent);
		}
