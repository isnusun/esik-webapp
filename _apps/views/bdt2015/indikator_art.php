			...
			<div class="box box-primary">
				...
				?>
					...
					<thead><tr><th>#</th>
						...
						<th>Data PBDT 2015</th>
					</tr></thead>
					<tbody>
					<?php
					$nomer=0;
					foreach($bdt_indikator['art'] as $key=>$rs){
						if($rs['jenis']=='pilihan'){
							$nomer++;
							echo "<tr><td class='angka'>".$nomer."</td>";
							if($rs['opsi']){
								echo "<td colspan='2'><a href='".site_url('backend/bdt2015/indikator_detail/art/'.$rs['id'])."/?kode=".$varKode."'>".$rs['nama']."</a></td>";
							}else{
								...
								if(is_array($rs['opsi'])){
									foreach($rs['opsi'] as $o=>$op){
										echo "<tr><td></td>
											...
											echo "<td class='angka'><a href='".site_url("bdt2015/art_mana/?indikator=".$key."&amp;opsi=".$o."&amp;kode=".$varKode)."'>".number_format($nilai,0)."</a></td>											
										</tr>";
									}
								}
							}
