										...
										<div id=\"collapse".$k."\" class=\"panel-collapse collapse \">
											<div class=\"box-body\">
												...
												foreach($pbk_param[$k] as $key=>$item){
													echo "<li><a href=\"".site_url('pbdt/indikator/'.$varKode.'/'.$item['id'].'/'.fixNamaUrl($item['nama']))."\">".$item['nama']."</a></li>";
												}
												echo "
												</ul>
											</div>
								...
								<table class="table table-responsive table-bordered datatables">
									...
									<tbody>
										...
										foreach($data_kelas[$d] as $key=>$item) {
											...
											if($item['tingkat'] < 7){
												echo "
												<tr><td><a href=\"".site_url('pbdt/indikator/'.$key.'/'.$indikator['id'].'/'.fixNamaUrl($indikator['nama']))."\">".$tingkatan[$item['tingkat']]." ".$item['nama']."</a></td>";
											}else{
												echo "
											...
											foreach($indikator_opsi as $o=>$ops){
												...
												echo "
												<td class=\"angka\"><a href=\"".site_url('pbdt/indikator_rtm/'.$key.'/'.$d.'/'.$ops['param_id'].'/'.$ops['id'].'/')."\">".number_format($nilai_idv,0)."</a></td>
												";
											}
											echo "
											</tr>
									...
									<tfoot>
										<tr><th class="angka">&Sigma; JUMLAH</th>
											...
											foreach($indikator_opsi as $key=>$item){
												echo "
												<th class=\"angka\"><a href=\"".site_url('pbdt/indikator_rtm/'.$varKode.'/'.$d.'/'.$item['param_id'].'/'.$item['id'].'/')."\">".number_format($sumIDV[$key],0)."</a></th>
												";
											}
											?>
										</tr>
