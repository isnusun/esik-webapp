				...
				<div class="box-body">
					...
					if(is_array($alamat)){
						foreach($alamat as $key=>$item){
							echo "<li><a href=\"".site_url('pbdt/indikator/'.$item['kode'].'/'.$indikator['id'].'/'.fixNamaUrl($indikator['nama']))."\">".$item['nama']."</a></li>";
						}
					}
					?>
					</ol>
						...
						<div class="box box-success">
							...
							<div class="box-body">
								...
								<table class="table table-responsive table-bordered datatables">
									...
									<tbody>
										<?php
										$toGraph["title"][0][$d] = "Grafik Data Sisiran ".$des['nama']." Rumah Tangga Per Wilayah dgn ".$boxTitle;
										$i = 1;
										...
										foreach($data_kelas[$d] as $key=>$item) {
											...
											if($item['tingkat'] < 7){
												echo "
												<tr><td><a href=\"".site_url('pbk/indikator/'.$key.'/'.$indikator['id'].'/'.fixNamaUrl($indikator['nama']))."\">".$tingkatan[$item['tingkat']]." ".$item['nama']."</a></td>";
											}else{
												echo "
											...
											foreach($indikator_opsi as $o=>$ops){
												...
												echo "
												<td class=\"angka\"><a href=\"".site_url('pbk/indikator_rtm/'.$key.'/'.$d.'/'.$ops['param_id'].'/'.$ops['id'].'/')."\">".number_format($nilai_idv,0)."</a></td>
												";
											}
											echo "
											</tr>
									...
									<tfoot>
										<tr><th class="angka">&Sigma; JUMLAH</th>
											...
											foreach($indikator_opsi as $key=>$item){
												echo "
												<th class=\"angka\"><a href=\"".site_url('pbk/indikator_rtm/'.$varKode.'/'.$d.'/'.$item['param_id'].'/'.$item['id'].'/')."\">".number_format($sumIDV[$key],0)."</a></th>
												";
											}
											?>
										</tr>
								...
								<div class="">
									<fieldset class="kotak">
										<legend><strong>Grafik Indikator "<?php echo $boxTitle . "\" ".$des['nama']; ?></strong> di <?php echo $wilayah;?></legend>
										<div>
								...
								<!--peta-->
									<fieldset class="kotak">
										<legend><strong>Peta Sebaran Data Sisiran </strong><?php echo $wilayah;?></legend>
										<div class="row">
    ...
    <script>
      ...
      function initMap() {
        ...
        map_<?php echo $d;?>.data.loadGeoJson(
            '<?php echo site_url('petajson/pbdt_indikator/'.$varKode.'/'.$indikator['id'].'/'.$des['id']); ?>');
