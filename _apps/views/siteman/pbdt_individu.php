				...
				<div class="box-body" >
					...
					if(is_array($alamat)){
						foreach($alamat as $key=>$item){
							echo "<li><a href=\"".site_url('pbdt/rtm/'.$item['kode'].'/'.$desil_aktif)."\">".$item['nama']."</a></li>";
						}
					}
					?>
					</ol>
					...
					if($rtm){
						if(count($rtm) > 0){
							...
							<thead><tr><th>#</th><th>No RTM</th><th>NIK</th><th>NAMA</th>
							<th>TGL LAHIR</th>
							<th>UMUR</th>
							<th>JENIS KELAMIN</th>
							<th>STATUS KAWIN</th>
							...
							foreach($rtm as $key=>$item){
								...
								<td><a href=\"".site_url('pbdt/rtm_view/'.$item['rtm_no'])."\">".$item['rtm_no']."</a></td>
								<td><a href=\"".site_url('pbdt/rtm_view/'.$item['rtm_no'])."\">".$item["nikkepala"]."</a></td>
								<td><a href=\"".site_url('pbdt/rtm_view/'.$item['rtm_no'])."\">".$item["kepala"]."</a></td>
								<td>".date("j M Y",strtotime($item["dtlahir"]))."</td>
								<td>".$item["umur"]."</td>
								<td>".$sex."</td>
								<td>".$item["kawin"]."</td>
								<td>".$item["alamat"]."</td>
								<td><a href=\"".site_url("pbdt/verivali/".$item["rtm_no"])."\"><i class=\"fa fa-newspaper-o\"></i></a></td>
								</tr>";
								$i++;
							}
							echo "</tbody>
