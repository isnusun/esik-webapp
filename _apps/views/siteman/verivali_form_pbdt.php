			...
			<div class="box box-primary">
				...
				<div class="box-body">
					<dl class="dl-horizontal">
						<?php 
							$dtlahir = ($rtm['kepala_dtlahir'] == '0000-00-00') ? "Tidak Terdata":date("j F Y",strtotime($rtm['kepala_dtlahir'])).", ".$rtm['kepala_umur']." tahun";
							echo "
 ...
 * */
	...
	<input type=\"hidden\" name=\"data_to\" value=\"pbdt\">
				...
				foreach($periodes['periode'] as $p=>$periode){
					if($indikator_data[$periode['id']]){
						...
						if($periode["status"] == 0){
							...
							echo "<td style=\"width:30%\"><div class=\"form-group has-success\"><label>".$datane."</label></div></td>";
							...
							$prevVal = $indikator_data[$periode['id']][$indi["kode"]];
								...
								echo "
								<td style=\"width:30%\"><div class=\"form-group has-warning\">";
								if($indi["jenis"]==1){
									echo "<select name=\"".$indi["kode"]."__".$indi["id"]."\" class=\"form-control validate[required]\">";
									foreach($indi["opsi"] as $o=>$ops){
										$strS = ($ops['kode'] == $prevVal) ? "selected=\"selected\"":"";
										echo "<option value=\"".$ops["kode"]."\" ".$strS.">".$ops["nama"]."</option>";
									}
									echo "</select>";
								}else{
									echo "<input name=\"".$indi["kode"]."__".$indi["id"]."\" id=\"\" class=\"form-control validate[required]\" value=\"".$prevVal."\"/></td>";
								}
								echo "</div>
								</td>";
						}
						...
						echo "
						<td style=\"width:30%\"><div class=\"form-group has-error\">";
						if($indi["jenis"]==1){
							echo "<select name=\"".$indi["kode"]."__".$indi["id"]."\" class=\"form-control validate[required]\">";
							foreach($indi["opsi"] as $o=>$ops){
								echo "<option value=\"".$ops["kode"]."\">".$ops["nama"]."</option>";
							}
							echo "</select>";
						}else{
