		...
		<section class="content">
			...
			<div class="box box-primary">
				...
				<div class="box-body">
					...
					<form action="<?php echo site_url('backend/pbdt/query_rts'); ?>" method="POST" role="form" class="formular">
					...
					if($_REQUEST){
						...
						foreach($pre_qs as $str){
							...
							if(!empty($d[1])){
								$dkey = $d[0];
								$dkey = str_replace('_min','',$dkey);
								$dkey = str_replace('_max','',$dkey);
								$string_query[$dkey][]=$d[1];
							}
						}
						...
						if($dataset){
							...
							if($paging['page_total']> 1){
								...
								if($paging['page_prev'] > 1){
									echo "<li><a href='".site_url('backend/pbdt/query_rts/?param='.$query_string)."&amp;p=1'><i class='fa fa-fast-backward'></i></a></li>
									<li><a href='".site_url('backend/pbdt/query_rts/?param='.$query_string)."&amp;p=".$paging['page_prev']."'><i class='fa fa-backward'></i></a></li>";
								}
								...
								for($i=$apage;$i<$epage;$i++){
									echo "<li><a href='".site_url('backend/pbdt/query_rts/?param='.$query_string)."&amp;p=".$i."'>".$i."</a></li>";
								}

								if($paging['page_next'] < $paging['page_total']){
									echo "<li><a href='".site_url('backend/pbdt/query_rts/?param='.$query_string)."&amp;p=".$paging['page_next']."'><i class='fa fa-forward'></i></a></li>
									<li><a href='".site_url('backend/pbdt/query_rts/?param='.$query_string)."&amp;p=".$paging['page_total']."'><i class='fa fa-fast-forward'></i></a></li>";
								}else{
									echo "<li><a href='".site_url('backend/pbdt/query_rts/?param='.$query_string)."&amp;p=".$paging['page_total']."'><i class='fa fa-fast-forward'></i></a></li>";
								}
								echo "</ul></div>";
							}							
						}else{
