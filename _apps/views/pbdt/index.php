		...
		<section class="content">
			...
			<div class="box box-primary">
				<div class="box-header with-border">
					<h3 class="box-title"><a href="<?php echo site_url('pbdt');?>">Telusur Data PBDT berbasis Wilayah Administrasi <?php echo $user['wilayah_nama']?></a></h3>
				...
				<div class="box-body">
					...
					<div class='callout callout-warning'>
						<h4>Perhatikan, pada kanan atas kotak ini, kesesuain data yang dikehendaki dengan periode pendataan</h4>
					</div>
					...
					if($sub_wilayah){
						...
						foreach($sub_wilayah as $w){
							...
							<td><a href='".site_url('backend/pbdt/')."?kode=".$w['kode']."'>".$tingkat_wilayah[$w['tingkat']]." ".strtoupper(strtolower($w['nama']))."</a></td>";
							$subtotal['rts'] = 0;
							$subtotal['art'] = 0;
							foreach($desil as $d){
								...
								echo "<td class='angka'><a href='".site_url('backend/pbdt/rts_desil/'.$d['id'].'/')."?kode=".$w['kode']."'>".number_format($nilai_rts,0)."</a></td>
								<td class='angka'><a href='".site_url('backend/pbdt/art_desil/'.$d['id'].'/')."?kode=".$w['kode']."'>".number_format($nilai_art,0)."</a></td>";
							}
							echo "
							<td class='angka total'><a href='".site_url('backend/pbdt/rts_desil/0/')."?kode=".$w['kode']."'>".number_format($subtotal['rts'],0)."</a></td>
							<td class='angka total'><a href='".site_url('backend/pbdt/art_desil/0/')."?kode=".$w['kode']."'>".number_format($subtotal['art'],0)."</a></td>
							</tr>";
...
<script>
	...
	// initialize Leaflet
	var map = L.map('map_bdt').setView({lon: <?php echo $varLon;?>, lat: <?php echo $varLat;?>}, 14);
